...
**Prerequisites**: Task 1 & 2 completed, site must be built (`pnpm run build`)  
**Context**: Read `docs/testing-context.md` first

## Objective

Create integration tests that validate the HTML structure generated by Hugo. These tests parse the built `public/` directory to verify pages have required elements.
...
**IMPORTANT**: Integration tests require the site to be built first.
